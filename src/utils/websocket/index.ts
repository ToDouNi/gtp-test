class WS {
  ...
  constructor(
    ...
    this.ip = ip
    this.url = url
    this.ws = new WebSocket(`ws://${ip}${url}`)
    this.closure = (): void => {
      this.ws.close()
    }
    ...
    const onopen_ = (ev: Event): any => {
      ...
      console.log(`websocket${this.ip}${this.url}连接开启！`)
    }
    ...
    const onmessage_ = (ev: MessageEvent): any => {
      ...
      console.log(`websocket${this.ip}${this.url}收到信息：${ev}！`)
    }
    ...
    const onerror_ = (ev: Event): any => {
      ...
      console.log(`websocket${this.ip}${this.url}连接error：${ev}！`)
    }
